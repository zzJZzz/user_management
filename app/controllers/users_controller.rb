...
class UsersController < ApplicationController
  ...
  def index
    ...
    @pagy, @users = pagy(@results.order(:id), items: 25)

    current_page = params[:page].to_i || 1
    return unless @users.empty?

    FetchUsersWorker.perform_async(current_page)
  end
  ...
  def user_params
    params.require(:user).permit(:first_name, :last_name, :email, :phone, :height, :weight)
  end
end
